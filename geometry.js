...
export function findNearestLink(links, points, p, minDistance) {
  ...
  let nearestDistance = distanceFromPointToLink(points, point, nearestLink)
    .distance;
...
function calcHinge({x: x0, y: y0}, {x: x1, y: y1}, {x: xt, y: yt}, l2t) {
...
export function platesIntersect(segments0, segments1, computedPoints) {
  for (const [p0, p1] of segments0) {
    for (const [p2, p3] of segments1) {
      if (
        lineSegmentsIntersect(...[p0, p1, p2, p3].map(p => computedPoints[p]))
      ) {
        return true;
      }
    }
