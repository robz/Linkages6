...
export function drawState(
  ...
  switch (state.type) {
    ...
    case 'slider_gp': {
      const p0 =
        typeof state.p0 === 'string' ? computedPoints[state.p0] : state.p0;
      const p1 =
        typeof state.p1 === 'string' ? computedPoints[state.p1] : state.p1;
  ...
  if (typeof hoverPoint === 'string') {
    ...
    const slider = linkage.links.find(
      link => link.type === 'slider' && link.p1 === hoverPoint,
    );
...
export function drawTraces(
  traces /*: {[pRef]: Array<Array<{x: number, y: number}>>} */,
  hoverPoint,
) {
...
export function drawStaticSVG(
  ...
  const drawSVGLink = (
    ...
    for (const dx of [holeOffset, d - holeOffset]) {
      drawSVGHole(xoffset + dx, yoffset + holeOffset, holeDiam, scale);
    }
  };
