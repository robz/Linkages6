...
function getLinksForPoint(linkage, p0Ref) {
    return linkage.links.filter(link => 
        link.p0 === p0Ref ||
        link.p1 === p0Ref ||
        link.p2 === p0Ref
    );
...
function getPointRefFromLinkID(linkID) { // {p0, xy}
...
function getPlates(linkage) {
    ...
    const getPlateIndex = ({p0, p1}) => plates.findIndex(plate => {
...
export function computePassThrus(plates, pointConnections, pointsList, margin=0.15) {
        ...
        for (const [plate, pointsToCheck] of platesToCheck.entries()) {
            for (const p of pointsToCheck) {
                for (const segment of plate.segments) {
                    const {distance} = distanceFromPointToLink(computedPoints, p, segment);
                    if (distance < margin) {
...
export function computePlanes(linkage, pointsList) {
    ...
    const intersections = getPlateIntersections(plates, pointsList, plateConnections);   
    const passThrus = computePassThrus(plates, pointConnections, pointsList);

    const plateToPlane = layerPlates(plates, plateConnections, intersections, passThrus)[0];
